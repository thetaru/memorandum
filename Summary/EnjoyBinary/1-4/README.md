...
アセンブラの場合、フラグを操作するcmp、test命令とフラグの値によって処理を分岐させるジャンプ系命令で実現する。  
こういった背景があるため、cpm、test命令の直後にジャンプ系命令が来ることがほとんどである。  
...
; eaxレジスタにvar_1004領域のアドレスを格納している?(この辺よくわからんので後日調べる)
; char var_1004[2048];
; eax = var_1004;
