...
これは変数strの実態がchar型の配列であり、配列を渡すことはその配列の先頭アドレスへのポインタ(&str[0])を渡すことに等しいという理由からです。  
つまりstrを渡していても実際には&str[0]を渡しているということです。(strと&strと&str[0]は同じ挙動をします。)
### 2.12.6 scanf使用時の注意点
