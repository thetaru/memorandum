...
`p++(p=p+1)`が`0x100+0x001=0x101`でなく`0x100+0x004=0x104`と計算していることがわかります。  
...
次に`*(p+X)`の部分を解説します。  
そもそも`*p`とは上の図でいうと`0x100`アドレスに格納されたデータのことなので`array[0]`のことです。  
  
次に`*(p+1)`を考えてみます。  
`6.6.3 ポインタを使った配列へのアクセス`でポインタのインクリメント`p++(p=p+1)`が次の要素を指すことを思い出してください。  
つまり`p+1`は`array[1]`のアドレス(0x104)を指しています。  
