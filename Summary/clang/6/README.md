...
ArrayのポインタはArrayの先頭アドレス0x100を指しています。
...
`p++`が`0x100+0x001=0x101`でなく`0x100+0x004=0x104`と計算していることがわかります。  
実はポインタのインクリメントは次の要素を指すようにしています。(つまり型の大きさ分のアドレスを進めます。)  
例えば、char型なら型の大きさが1なので1アドレス分進みます。
## 6.7 配列とポインタ
ポインタと配列の違いについて
### 6.7.1 配列とポインタ入門
### 6.7.2 配列とポインタ
