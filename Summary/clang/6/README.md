...
※ ポインタ変数`x`、`y`の中身自体は変更されないことに注意してください。`x`と`y`の指す先の変数が変更されているだけです。
## 6.6 ポインタの型
変数の型の大きさについて学びます。
### 6.6.1 sizeof
...
加えて、図で(というか宣言の仕方から)わかるとおり文字列定数そのものには変数名がついていないのでポインタを使わない限りアクセスできない変数です。
### 6.8.2 文字列定数と文字配列の違い
...
```
`str[0]`は`*(str + 0)`のことで文字列定数"Hello"の先頭アドレスのデータ'H'を指しています。
しかし、`str`は文字列定数なので値の書き換えはできません。したがってエラーとなります。  
一方、`str2`は変数なので自由に書き換えることができます。  
### :warning:なぜ書き換えるとエラーになるのか:warning:
そもそも`str[0]`は`*(str + 0)`に等しく、意味は指しているアドレスに格納されているデータのことです。  
