...
inc インクリメント対象
```
inc命令は2つの機械語命令が1つのアセンブリ言語命令に対応する例外的な命令です。

## 機械語の構造
各要素は、すべての命令に必須のものや、命令によって省略されるものがあります。
...
inc dword[ebp-0x4]
```
mov命令と同じく、変数valを指定する部分は`dword [ebp-0x4]`です。  
変数valに1を加算します。
...
# 1.8 2の補数入門
2の補数表現を使って負数に変換するには、変換対象の整数より1桁だけ大きい2のべき数から元の整数を引けば求まります。  
※ 全ビットを反転させて+1(2)しても同じ結果を得られる。

## 例1
```
41 (10) = 00101001 (2) を2の補数表現を使って負数に変換します。
...
計算結果から11010111 (2)は2の補数表現で-41 (10)を表すことがわかりました。

※ もう一つの求め方: ~(00101001) (2) + 1 (2) = 11010110 (2) + 1 (2) = 11010111 (2)
```
ただ、これを正の整数とみなすと215 (10)とも解釈できてしまいます。  
そのため、最上位ビットが1の2進数をみたら、**符号つき整数**か**符号なし整数**かを意識しなければ、正しい数値を得られないというわけです。
