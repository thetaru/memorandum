...
と等価です。  
1.で、スタックの先頭がスタックフレームの先頭まで巻き戻されます。  
2.で、巻き戻った先でスタックの先頭に古いebpが積んである状態になり、popでその値を取り除きます。  
こうすることで、call命令で関数の先頭にジャンプした直後と同じスタックの状態になります。

## 3.9 フラグレジスタと条件分岐命令
ある条件を満たすときだけジャンプする命令を条件分岐命令と呼びます。  
まず、条件分岐命令の動作の元になるフラグレジスタを説明します。  
|ビット|名前|役割|
|:---|:---|:---|
|0|CF|キャリーフラグ</br>add/subで最上位ビットで繰り上がり/繰り下がりが発生した場合に1となる|
|6|ZF|ゼロフラグ</br>演算結果が0なら1となる|
|7|SF|符号フラグ</br>演算結果の最上位ビットがコピーされる|
|11|OF|オーバーフローフラグ</br>符号付き演算で格納先フラグが表せる値の範囲を超えた場合に1となる|

x86 CPUはeflagsというレジスタを持ちます。このレジスタはビットごとに意味があります。  
eflagsは、算術論理演算命令(add,sub,and,orなど)の実行結果に応じて書き換わります。 
  
条件分岐命令は、各フラグの状態に応じてジャンプするかしないかが決まるジャンプ命令です。  
1つのフラグのみを条件にする命令(js,jc,jz,joなど)もあれば複数のフラグを条件にする命令(ja,jgなど)もあります。  
  
条件分岐命令のために、(sub命令の代わりに)オペランドの大小関係を比較するcmp命令を使います。

## 3.10 エミュレータの条件分岐命令対応
cmp命令を実装し、eflagsを使ってjz,jgなどの条件分岐命令を作ります。

## 3.11 プログラムの繰り返し
forやwhileなどの繰り返し文を考えます。  
