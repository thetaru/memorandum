...
## 1.2 PIC16F84
### 1.2.1 PIC16F84の特徴
PIC16F84は、18ピンの小型8ビットマイコンであり、以下の特徴を持つ。
### ■ 制御向けマイクロコントローラ
書籍参照
### ■ 豊富な機能
書籍参照
...
### 1.2.2 PIC16F84の基本構成
### ■ 基本構成
書籍よりPIC16F84の構成図(図1.11)を参照
### ■ データ構造
PIC16F84が一度に扱えるデータは、8ビットのデータであり、構成図のデータバスを経由して、各装置間を伝わる。
### ■ 命令の形式
すべての命令は、14ビットよりなる1語のデータによって構成される。  
14ビット幅の命令は、命令の内容(オペコード)と命令の対象(オペランド)で分かれる。  
※ オペコードは固定長ではないことに注意
### ■ 命令サイクル
一つの命令の実行には、命令のフェッチに4サイクル、命令の解読・実行に4サイクルを必要とする。  
実際は、パイプライン方式により命令の実行と次の命令のフェッチを並行して行うので、4サイクルで1命令(CPI=4)が実行される。
```
       ------+      +------+      +------+      +------+      +------+      +------+      +------+      +------+      +------+      +------+      +------+      +------+
             |      |      |      |      |      |      |      |      |      |      |      |      |      |      |      |      |      |      |      |      |      |      |
             +------+      +------+      +------+      +------+      +------+      +------+      +------+      +------+      +------+      +------+      +------+      +------+
...
### ■ プログラムメモリ
- プログラムメモリの役割
> プログラムメモリは、プログラムを格納する専用のメモリで、フラッシュメモリで構成される。  
> プログラムメモリのメモリ領域は、データ幅14ビットで、0000(16)番地から03FF(16)番地までの1Kワード(1024ワード)である。  
> リセット時は、プログラムカウンタが0番地(リセットベクタ)となり、0番地に格納されている命令より実行が開始される。  
> 割り込み時には、プログラムカウンタは4番地(割り込みベクタ)となる。

### ■ Wレジスタとファイルレジスタ
- レジスタとは
> レジスタはデータを一時的に格納する装置である。  
> レジスタにデータを転送すると、古いデータは更新される。  
> レジスタからデータを取り出しても、レジスタに格納されているデータは保持される。
- Wレジスタ
> 演算結果を格納する演算用のレジスタで、アキューミュレータともいう。  
> ALUとWレジスタの両方を使用して演算を行う。
- ファイルレジスタ
> メモリと同様に、アドレスを指定して使用する。  
> 特殊機能レジスタと汎用レジスタにより構成される。  
> 00(16)～0B(16), 80(16)～8B(16)番地までのファイルレジスタは、特殊レジスタと呼ばれ、マイコンの機能やモードを設定したり、マイコンの状態を示したりする。  
> 0C(16)～4F(16)番地までは、汎用ファイルレジスタと呼び、内蔵SRAMで構成され、ユーザが自由に使用できる。

※ バンクについては書籍を参照
- フラグレジスタ
> 1ビットのレジスタで、演算結果の状態によって、セット("1")またはリセット("0")される。  
> 演算結果の状態を保持するので、条件により分岐する命令などに使用される。

|フラグ名|STATUSレジスタのビット|セット("1")になる条件|リセット("0")になる条件|
|:---|:---|:---|:---|
|Cフラグ|0|演算の結果、桁あふれが発生|演算の結果、桁あふれが発生しない|
|DCフラグ|1|演算の結果、下位4ビット目から桁あふれが発生|演算の結果、下位4ビット目から桁あふれが発生しない|
|Zフラグ|2|演算結果が0|演算結果が0以外|
