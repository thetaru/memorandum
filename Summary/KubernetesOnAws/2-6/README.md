...
CloudFormationのテンプレートを用意してあるので、マネジメントコンソールのCloudFormationより`スタックの作成`-`新しいリソースを使用(標準)`を押して、スタックの作成を開始してください。  
`スタックの作成`画面では、`テンプレートの準備完了`と`テンプレートファイルのアップロード`を選び、テンプレートファイルとして`40_s3_batch_cfn.yaml`を指定して、`次へ`を押します。
