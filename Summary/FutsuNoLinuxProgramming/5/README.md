...
カーネルが持っているストリームと番号(i.e. ファイルディスクリプタ)を対応させることで、プロセスは番号からストリームを指定できるようになります。  
  
以上のように、ストリームと番号(ファイルディスクリプタ)の対応関係が重要になってきます。
## 5.3 標準入力、標準出力、標準エラー出力
ストリームとファイルディスクリプタの対応関係のうち事前に定められた関係が存在します。
...
## ■ 標準入力と標準出力
コマンドは一般的に標準入力からデータを読み込み、処理結果を標準出力へ書き込むようになっています。  
...
そのため、端末に繋がっている可能性がより高いストリームを余分に用意して、プログラムに渡したい出力は標準出力に、人間に読ませたい出力は標準エラー出力に出すようにします。
...
|bufsize|ファイルから読み込むバイト数|

|戻り値|意味|
|:---|:---|
|成功|読み込んだバイト数</br>ファイルの終端に達した場合は0|
|失敗|-1|

read()は、ファイルディスクリプタfd番のストリームからbufsizeで指定した領域分のバイト列を読み込み、bufへ格納します。  
...
write()は、bufに格納されているbufsizeバイト分のデータをファイルディスクリプタfd番のストリームに書き込みます。  
...
|フラグ|意味|
|:---|:---|
|O_APPEND|ファイルポインタをファイルの最後に移動する(ファイル末尾に書き込める)|
|O_CREAT|ファイルが存在しなければファイルを生成する|
|O_TRUNC|ファイルが存在するときはファイルのサイズをゼロにする|
...
open()は、pathで表されるファイルにつながるストリームを作成し、そのストリームを指すファイルディスクリプタを返します。
## ■ close(2)
...
close()は、ファイルディスクリプタfd番のストリームを始末します。
## 5.6 catコマンドを作る
## 5.6.1 cat.c
...
標準エラー出力に対して、エラーメッセージstrとerrnoに対応するシステムエラーメッセージを出力します。  
文字列strがNULLでも空文字でもないときはstrと":"を出力し、そのあとにメッセージを出力します。
## 5.7 その他のシステムコール
## ■ ファイルオフセット
...
|:---|:---|
|fd|ファイルディスクリプタ|
|offset|whenceで指定した起点から何バイト離れているかを指定する|
|whence|下記の表を参照|

|whence|移動先|起点|
|:---|:---|:---|
|SEEK_SET|先頭からoffsetバイト離れた場所に移動|ファイル先頭|
|SEEK_CUR|現在位置からoffsetバイト離れた場所に移動|現在位置|
|SEEK_END|ファイルサイズにoffsetバイト加えた場所に移動|ファイル末尾|

|戻り値|意味|
|:---|:---|
|成功|ファイルの先頭から結果のオフセット位置をバイトで返す|
|失敗|-1|
