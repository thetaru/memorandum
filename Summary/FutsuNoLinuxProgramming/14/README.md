...
## 14.3 クレデンシャルの操作
### ■ set-uid プログラム
まずset-uidプログラムについて説明します。  
何らかの理由で、コマンドを実行するユーザに関係なく特定のユーザ権限で実行したいということがあります。(例えば、パスワードを変更するpasswdコマンドなど)  
そういうときに使われるのが、ファイルパーミッションのset-uidビットです。  
常に特定の権限で実行したいプログラムにset-uidビットをセットしておくと、起動したユーザにかかわらず、プログラムファイルのオーナー権限で起動されます。  
...
set-uidプログラムから起動されたプロセスには、2種類のクレデンシャルが存在します。  
つまり、プロセスを起動したユーザと、set-uidプログラムのオーナーです。  
