...
基本的に、親プロセスとfork()された子プロセスで構成される。
#### セッション
...
setsid()は、新しいセッションを作成します。実行プロセスのPIDがSIDとPGIDに代入され、新しいセッションのセッションリーダー兼プロセスグループリーダーとなります。  
また、このプロセスは制御端末を持たない状態になります。  
  
自プロセスがすでにプロセスグループリーダーになっている場合は失敗するので、あらかじめfork()してプロセスグループリーダーでなくす必要があります。  
要は、子プロセスを作成し、それを(セッション兼プロセスグループ)リーダーとして立てることで親プロセスと分離(セッションを分けることが)できる。

## 12.5 練習問題
1. fork()した親プロセスと子プロセスのSIDを確認してみよう。
2. fork()とexec()を使ってプログラムを起動する、シェルを書いてみよう。
